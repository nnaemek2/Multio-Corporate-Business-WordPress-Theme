...
if (!function_exists('multio_setup')) :
    /**
     ...
     */
    function multio_setup()
    {
        // Make theme available for translation.
        load_theme_textdomain('multio', get_template_directory() . '/languages');
        ...
        register_nav_menus(array(
            'primary' => esc_html__('Primary', 'multio'),
            'top-bar' => esc_html__('Top Bar', 'multio'),
        ));
        ...
        add_theme_support('custom-background', apply_filters('multio_custom_background_args', array(
        ...
        add_image_size( 'multio-large', 980, 580, true );
        add_image_size( 'multio-medium', 680, 456, true );
        add_image_size( 'multio-gallery', 480, 430, true );

        remove_theme_support('widgets-block-editor');

    }
endif;
add_action('after_setup_theme', 'multio_setup');
 ...
 */
function multio_content_width()
{
    $GLOBALS['content_width'] = apply_filters('multio_content_width', 640);
}

add_action('after_setup_theme', 'multio_content_width', 0);

/**
 * Register widget area.
 */
function multio_widgets_init()
{
    register_sidebar(array(
        'name'          => esc_html__('Blog Sidebar', 'multio'),
        'id'            => 'sidebar-blog',
        'description'   => esc_html__('Add widgets here.', 'multio'),
    ...
    register_sidebar(array(
        'name'          => esc_html__('Page Sidebar', 'multio'),
        'id'            => 'sidebar-page',
        'description'   => esc_html__('Add widgets here.', 'multio'),
        'before_widget' => '<section id="%1$s" class="widget %2$s"><div class="widget-content">',
        'after_widget'  => '</div></section>',
    ...
    if($hidden_sidebar_on) {
        register_sidebar(array(
            'name'          => esc_html__('Hidden Sidebar', 'multio'),
            'id'            => 'sidebar-hidden',
            'description'   => esc_html__('Add widgets here.', 'multio'),
            'before_widget' => '<section id="%1$s" class="widget %2$s"><div class="widget-content">',
            'after_widget'  => '</div></section>',
    ...
    if(class_exists('Woocommerce')){
        register_sidebar(array(
            'name'          => esc_html__('Shop Sidebar', 'multio'),
            'id'            => 'sidebar-shop',
            'description'   => esc_html__('Add widgets here.', 'multio'),
            'before_widget' => '<section id="%1$s" class="widget %2$s"><div class="widget-content">',
    ...
    if (class_exists('ReduxFramework')) {
        ...
        $footer_top_column = multio_get_opt( 'footer_top_column', '4' );
        for($i = 1 ; $i <= $footer_top_column ; $i++){
            register_sidebar(array(
                'name' => sprintf(esc_html__('Footer Top %s', 'multio'), $i),
                'id'            => 'sidebar-footer-' . $i,
                'description'   => esc_html__('Add widgets here.', 'multio'),
...
add_action('widgets_init', 'multio_widgets_init');

/**
 * Enqueue scripts and styles.
 */
function multio_scripts()
{
    ...
    wp_enqueue_style('multio-theme', get_template_directory_uri() . '/assets/css/theme.css', array(), $theme->get('Version'));
    wp_enqueue_style('multio-menu', get_template_directory_uri() . '/assets/css/menu.css', array(), $theme->get('Version'));
    wp_enqueue_style('multio-style', get_stylesheet_uri());
    wp_enqueue_style('multio-google-fonts', multio_fonts_url());
    ...
    wp_enqueue_script('multio-sidebar-fixed', get_template_directory_uri() . '/assets/js/sidebar-scroll-fixed.js', array( 'jquery' ), '1.0.0', true);
    wp_enqueue_script('magnific-popup', get_template_directory_uri() . '/assets/js/magnific-popup.min.js', array('jquery'), '1.0.0', true);
    wp_enqueue_script('multio-main', get_template_directory_uri() . '/assets/js/main.js', array('jquery'), $theme->get('Version'), true);
    wp_register_script('multio-carousel', get_template_directory_uri() . '/assets/js/ct-carousel.js', array('jquery'), $theme->get('Version'), true);
    wp_register_script('multio-carousel-filter', get_template_directory_uri() . '/assets/js/owl-filter.js', array('jquery'), $theme->get('Version'), true);
    wp_register_script('multio-counter-lib', get_template_directory_uri() . '/assets/js/counter.min.js', array('jquery'), $theme->get('Version'), true);
    wp_register_script('multio-counter', get_template_directory_uri() . '/assets/js/ct-counter.js', array('jquery'), $theme->get('Version'), true);
    $smoothscroll = multio_get_opt( 'smoothscroll', false );
    ...
    wp_localize_script('multio-main','main_data',array('ajax_url'=>admin_url('admin-ajax.php')));
}

add_action('wp_enqueue_scripts', 'multio_scripts');

/* add editor styles */
function multio_add_editor_styles()
{
    add_editor_style('editor-style.css');
}

add_action('admin_init', 'multio_add_editor_styles');

/* add admin styles */
function multio_admin_style()
{
    $theme = wp_get_theme(get_template());
    wp_enqueue_style('multio-admin-google-fonts', multio_fonts_url());
    wp_enqueue_style('multio-admin-style', get_template_directory_uri() . '/assets/css/admin.css');
    wp_enqueue_style('font-awesome5', get_template_directory_uri() . '/assets/css/font-awesome5.min.css', array(), '5.8.0');
    wp_enqueue_style('font-material-icon', get_template_directory_uri() . '/assets/css/material-design-iconic-font.min.css', array(), '2.2.0');
    wp_enqueue_style('flaticon', get_template_directory_uri() . '/assets/css/flaticon.css', array(), '1.0.0');
    wp_enqueue_style('themify-icons', get_template_directory_uri() . '/assets/css/themify-icons.css', array(), '1.0.0');
    wp_enqueue_style('font-etline-icon', get_template_directory_uri() . '/assets/css/et-line.css', array(), '1.0.0');
    wp_enqueue_script('multio-main-admin', get_template_directory_uri() . '/assets/js/main-admin.js', array( 'jquery' ), $theme->get('Version'), true);
}

add_action('admin_enqueue_scripts', 'multio_admin_style');
...
add_action('vc_after_init', 'multio_vc_after_init');

/**
 * Add new elements for VC
 */
function multio_vc_elements()
{
...
add_action('vc_before_init', 'multio_vc_elements');
...
require_once get_template_directory() . '/widgets/widget-fancybox.php';
 ...
 */
function multio_vc_shortcode_css_class( $classes, $settings_base, $atts )
{
...
if ( class_exists( 'WPBakeryShortCode' ) ) {
    add_filter( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'multio_vc_shortcode_css_class', 10, 3 );
}


if ( ! function_exists( 'multio_fonts_url' ) ) :
    /**
     * Register Google fonts.
     *
     * Create your own multio_fonts_url() function to override in a child theme.
     *
     * @since league 1.1
     *
     * @return string Google fonts URL for the theme.
     */
    function multio_fonts_url()
    {
        $fonts_url = '';
        $fonts     = array();
        $subsets   = 'latin,latin-ext';

        if ( 'off' !== _x( 'on', 'Heebo font: on or off', 'multio' ) )
        {
            $fonts[] = 'Heebo:300,400,500,600,700';
        }

        if ( 'off' !== _x( 'on', 'Muli font: on or off', 'multio' ) )
        {
            $fonts[] = 'Muli:300,400,500,600,700';
        }

        if ( 'off' !== _x( 'on', 'Poppins font: on or off', 'multio' ) )
        {
            $fonts[] = 'Poppins:300,400,500,600,700';
        }

        if ( 'off' !== _x( 'on', 'Playfair Display font: on or off', 'multio' ) )
        {
            $fonts[] = 'Playfair Display:400,400i,700,700i';
        }

        if ( 'off' !== _x( 'on', 'Open Sans font: on or off', 'multio' ) )
        {
            $fonts[] = 'Open Sans:400,500,600,700';
        }

        if ( 'off' !== _x( 'on', 'Roboto font: on or off', 'multio' ) )
        {
            $fonts[] = 'Roboto:400';
        }
 ...
 * Commnet Form
 */
function multio_comment_field_to_bottom( $fields ) {
    $comment_field = $fields['comment'];
    unset( $fields['comment'] );
    $fields['comment'] = $comment_field;
    return $fields;
}
add_filter( 'comment_form_fields', 'multio_comment_field_to_bottom' );
...
/* Favicon */
function multio_site_favicon(){
    
    $favicon = multio_get_opt( 'favicon' );
    
    if(!empty($favicon['url']))
        echo '<link rel="icon" type="image/png" href="'.esc_url($favicon['url']).'"/>';
}
add_action('wp_head', 'multio_site_favicon');

/**
 * Menu Item Active on Archive
 */
function multio_additional_active_item_classes( $classes = array(), $menu_item = false ) {
    if(class_exists('Woocommerce')){
...
if(class_exists('Woocommerce')){
    add_filter( 'nav_menu_css_class', 'multio_additional_active_item_classes', 10, 2 );
}