...
$post_tags_on = multio_get_opt( 'post_tags_on', true );
$post_feature_image_on = multio_get_opt( 'post_feature_image_on', true );
?>
    ...
    <div class="entry-blog">
        <?php if (has_post_format('gallery')) : ?>
            <div class="entry-featured entry-gallery">
                <?php
                $light_box = multio_get_post_format_value('post-gallery-lightbox', '0');
                $gallery_list = explode(',', multio_get_post_format_value('post-gallery-images', ''));
                wp_enqueue_script( 'owl-carousel' );
                wp_enqueue_script( 'multio-carousel' );
                ?>
                <div class="ct-carousel owl-carousel featured-active <?php if($light_box) {echo 'images-light-box';} ?>" data-item-xs="1" data-item-sm="1" data-item-md="1" data-item-lg="1" data-item-xl="1" data-margin="30" data-margin-md="30" data-loop="true" data-autoplay="false" data-autoplaytimeout="5000" data-smartspeed="250" data-center="false" data-arrows="true" data-bullets="false" data-stagepadding="0" data-stagepadding-xl="0" data-stagepadding-lg="0" data-rtl="<?php echo esc_attr( $carousel_rtl ); ?>">
                    <?php
        ...
        <?php elseif (has_post_format('quote')) : ?>
            <div class="entry-featured">
                <?php
                    $quote_text = multio_get_post_format_value('post-quote-cite', '');
        ...
        <?php elseif (has_post_format('link')) : ?>
            <div class="entry-featured">
                <?php
                    $link_pf = multio_get_post_format_value('post-link-url', '#');
        ...
        <?php elseif (has_post_format('video')) : ?>
            <div class="entry-featured">
                <div class="entry-video featured-active">
                    <?php
                    $video_url = multio_get_post_format_value('post-video-url', '#');
                    $video_file = multio_get_post_format_value('post-video-file', '');
                    $video_html = multio_get_post_format_value('post-video-html', '');
                    $video = '';
                    if (!empty($video_url)) {
                        global $wp_embed;
        ...
        <?php elseif (has_post_format('audio')) : ?>
            <div class="entry-featured">
                <?php
                    $audio_url = multio_get_post_format_value('post-audio-url', '#');
        ...
        <?php else :
            if (has_post_thumbnail() && $post_feature_image_on ) {
                echo '<div class="entry-featured"><div class="post-image">'; ?>
                    <?php the_post_thumbnail('multio-large'); ?>
                <?php echo '</div></div>';
            } ?>
        <?php endif; ?>
        <div class="entry-body">
            <?php multio_post_meta(); ?>
    ...
    <div class="entry-footer">
        <?php if($post_tags_on || $post_social_share_on) : ?>
            <div class="entry-content-bottom">
                <?php if($post_tags_on) :
                    multio_entry_tagged_in();
                endif;
                if($post_social_share_on) :
                    multio_socials_share_default(); 
                endif; ?>
            </div>
        <?php endif; ?>
        <?php multio_post_nav_default(); ?>
    </div>
</article><!-- #post -->