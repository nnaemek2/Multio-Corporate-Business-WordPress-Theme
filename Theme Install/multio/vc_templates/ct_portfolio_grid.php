...
<div id="<?php echo esc_attr($html_id) ?>" class="ct-grid ct-grid-portfolio-layout1 <?php echo esc_attr($el_class); ?>">

    <?php if ($filter == "true" and $layout == 'masonry'): ?>
        <div class="grid-filter-wrap <?php echo esc_attr($filter_style); ?>">
            <span class="filter-item active" data-filter="*">
                <?php echo esc_html($filter_default_title); ?>
                <i>
                    <?php
                        if($pagination_type !== 'pagination' && $pagination_type !== 'loadmore') {
                            echo '('.count($posts).')';
                        } else {
                            echo '('.esc_attr($total).')';
                        }
            ...
            <?php foreach ($categories as $category): ?>
                ...
                <span class="filter-item" data-filter="<?php echo esc_attr('.' . $term->slug); ?>">
                    ...
                    <i>
                        <?php
                            if($pagination_type !== 'pagination' && $pagination_type !== 'loadmore') {
                                echo '('.multio_get_post_count_of_term_filter($posts, 'portfolio', $term->term_id, $category_arr[1]).')';
                            } else {
                ...
                <div class="<?php echo esc_attr($item_class . ' ' . $filter_class); ?>">
                    <div class="grid-item-inner">
                        ...
                        <div class="item-holder">
                            ...
                            <a class="item-readmore" href="<?php echo esc_url(get_permalink( $post->ID )); ?>"><i class="fac fac-arrow-right"></i></a>
                        </div>
                    </div>
                </div>
            <?php $i++; } 
    ...
    <?php if (!empty($next_link) && $layout == 'masonry' && $pagination_type == 'loadmore') { ?>
        <div class="ct-load-more text-center">
            <span class="btn">
                <i class=""></i>
                <?php echo esc_html__('Load more', 'multio') ?>
            </span>
        </div>
    <?php } ?>
