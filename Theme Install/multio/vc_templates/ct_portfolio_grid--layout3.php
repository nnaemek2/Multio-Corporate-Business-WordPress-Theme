...
<div id="<?php echo esc_attr($html_id) ?>" class="ct-grid ct-grid-portfolio-layout3 <?php echo esc_attr($el_class); ?>">

    <?php if ($filter == "true" and $layout == 'masonry'): ?>
        <div class="grid-filter-wrap style-grid <?php echo esc_attr($filter_style); ?>">
    ...
    <div class="<?php echo esc_attr($grid_class); ?>" data-gutter="<?php echo esc_attr($gap_item); ?>" style="margin-top:-<?php echo esc_attr($gap_item).'px'; ?>">
        ...
        if (is_array($posts)):
            ...
            foreach ($posts as $key => $post) {
                ...
                if($key == 0) {
                    $item_class   = "grid-item grid-item-first col-xl-6 col-lg-6 col-md-6 col-sm-{$col_sm} col-xs-{$col_xs}";
                } else {
